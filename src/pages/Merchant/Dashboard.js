...
import AtlasChatModal from '../../features/atlas/ui/AtlasChatModal';
...
const Dashboard = () => {
  ...
  const [isAtlasChatOpen, setIsAtlasChatOpen] = useState(false);
  ...
  return (
    <div className={`dashboard ${isSidebarCollapsed ? 'sidebar-collapsed' : ''} ${isMobileMenuOpen ? 'mobile-menu-open' : ''}`}>
      ...
      {/* نافذة أطلس المنبثقة */}
            {isAtlasChatOpen && (
        <AtlasChatModal 
          onClose={() => setIsAtlasChatOpen(false)} 
          darkMode={darkMode} 
        />
      )}
      ...
      {/* ✅ زر شات عائم */}
      <button
        className="floating-atlas-btn"
        onClick={() => setIsAtlasChatOpen(true)}
        aria-label="فتح مساعد أطلس"
      >
        💬
      </button>
